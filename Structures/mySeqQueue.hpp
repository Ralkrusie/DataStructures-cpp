...
class mySeqQueue {
public:
	mySeqQueue() : data(nullptr), head(0), size(0), capacity(0) {
		reserve(8);
	}

	explicit mySeqQueue(size_t initialCapacity) : data(nullptr), head(0), size(0), capacity(0) {
	...
	mySeqQueue(const mySeqQueue &) = delete;
	mySeqQueue &operator=(const mySeqQueue &) = delete;
